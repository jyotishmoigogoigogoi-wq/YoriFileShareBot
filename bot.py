...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL1:
            try:
                ...
                self.LOGGER(__name__).info("\nBot Stopped. https://t.me/youryori7 for support")
                sys.exit()
        ...
        try:
            ...
            test = await self.send_message(chat_id = db_channel.id, text = "@YoriFederation file share test")
            await test.delete()
        except Exception as e:
            self.LOGGER(__name__).warning(e)
            self.LOGGER(__name__).warning(f"Make Sure bot is Admin in DB Channel, and Double check the CHANNEL_ID Value, Current Value {CHANNEL_ID}")
            self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/youryori7 for support")
            sys.exit()

        self.set_parse_mode(ParseMode.HTML)
        self.LOGGER(__name__).info(f"Bot Running..!\n\nCreated by \nhttps://t.me/youryori7")
  ...
  YoriFederation 
        ...
        self.LOGGER(__name__).info(f"Bot Running..! Made by @YoriFederation")   
